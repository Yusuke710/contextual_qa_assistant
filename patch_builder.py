# modification of gpt-index/indices/response/builder.py so that it does not refine answers
# quiicker and stable, works well with top_k similarities index
...
class ResponseBuilder:
    ...
    def give_response_single(
        ...
        buffer = []
        for cur_text_chunk in text_chunks:
            buffer.append(cur_text_chunk)
        aggregated_text_chunk = " ".join(buffer)

        ##print('printing aggregated_text[[[[[[[[[[[[[[[[[[[[[[')
        ##print(aggregated_text_chunk)
        
        if response is None:
            response, _ = self.llm_predictor.predict(
                text_qa_template,
                context_str=aggregated_text_chunk,
    ...
    def get_response_over_chunks(
        ...
        for text_chunk in text_chunks:
            ...
            ##print(text_chunk)
            buffer.append(text_chunk.text)
        aggregated_text = " ".join(buffer)
